...
interface FormularioFonteProps {
  ...
  documentGroupItems: DocumentGroupItem[];
  setValue: any;
  control: any;
  register: any;
  errors: any;
...
export default function Formulario({
  ...
  return (
    <form className="flex text-lg flex-col gap-4">
      ...
      <p className="flex flex-col gap-2">
        <Label className="text-lg">Grupo de documentos</Label>
          ...
          render={({ field }) => (
            ...
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Selecione um grupo de documentos" />
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
      ...
      <p className="flex flex-col gap-2">
        <Label className="text-lg">Tipo de documento</Label>
        ...
        <Controller
          ...
          render={({ field }) => (
            <Select
              value={field.value ?? ""}
              onValueChange={(value) => field.onChange(value)}
              disabled={documentGroupItems.length === 0}
            >
              <SelectTrigger className="w-full">
              ...
              <SelectContent>
                <SelectGroup>
                  ...
                  {documentGroupItems && documentGroupItems.length > 0 ? (
                    ...
                    <SelectItem value="no-item" disabled>
                      Selecione primeiro um grupo de documentos
                    </SelectItem>
                  )}
                </SelectGroup>
              </SelectContent>
      ...
      <p className="flex flex-col gap-2">
                ...
                <SelectGroup>
                  ...
                  {fontes &&
                    fontes
                      .filter(
                        (f) =>
                          !fontesSelecionadas.some(
                            (fonte: Fonte) => fonte.id === f.id,
                          ),
                      )
                      .map((fonte, index) => (
                        <SelectItem
        ...
        {errors.fontesSelecionadas && (
          <span className="text-red-500 text-sm italic">
            {errors.fontesSelecionadas.message}
          </span>
        )}
      </p>
            ...
            {fontesSelecionadas.map((fonte: Fonte) => (
              ...
              >
                <div
                  ...
                  onClick={() => {
                    const novaLista = fontesSelecionadas.filter(
                      (f: Fonte) => f.id !== fonte.id,
                    );
                    setFontesSelecionadas(novaLista);
                    setValue(
                      "fontesSelecionadas",
                      novaLista.map((f) => f.id),
                    );
                  }}
                >
                  <div
