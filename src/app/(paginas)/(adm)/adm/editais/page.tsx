...
export default function Editais() {
  ...
  const getEditais = useCallback(async () => {
    try {
      const documentos = await getEditaisService(usuario?.unit_id);

      if (!documentos) {
        throw new Error();
      }

      const novasColunas: Record<StatusEdital, KanbanItem[]> = {
        PENDING: [],
        UNDER_CONSTRUCTION: [],
        WAITING_FOR_REVIEW: [],
        COMPLETED: [],
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (
      ...
      overContainer === "WAITING_FOR_REVIEW"
    ) {
      toast.info(
        "Não é permitido mover de rascunho direto para em análise. É necessário passar por em construção primeiro.",
      );
      return;
    }
    ...
    if (
      // Somente ADM e ANALIST podem mover de RASCUNHO -> EM CONSTRUÇÃO -> EM ANALISE
      ((activeContainer === "PENDING" &&
        (overContainer === "UNDER_CONSTRUCTION" ||
          overContainer === "WAITING_FOR_REVIEW" ||
          overContainer === "COMPLETED")) ||
        (activeContainer === "UNDER_CONSTRUCTION" &&
          (overContainer === "PENDING" ||
            overContainer === "WAITING_FOR_REVIEW" ||
            overContainer === "COMPLETED")) ||
        (activeContainer === "WAITING_FOR_REVIEW" &&
          overContainer === "PENDING")) &&
      !(
      ...
      toast.info(
        "Você não tem permissão para mover este edital para esta coluna.",
      );
      return;
    }
    ...
    <div className="flex flex-col h-full relative gap-4">
                ...
                return (
                  <div>
                    ...
                    <div className="min-w-0 wrap-break-word">
                      ...
                      <p className="text-sm text-gray-600">
                        {item.editors && item.editors.length > 1
                          ? "Responsáveis"
                          : "Responsável"}
                        :{" "}
                        {(item.editors ?? []).map((e) => e.username).join(", ")}
                      </p>
                    </div>
                  </div>
                );
      ...
      <Dialog open={!!pendingMove} onOpenChange={cancelMove}>
        <DialogContent>
          ...
          <p>
            Tem certeza que deseja mover{" "}
            <strong>{pendingMove?.item.name}</strong> para{" "}
            <strong>{formatStatus(pendingMove?.to || undefined)}</strong>?
          </p>
          ...
          <DialogFooter className="flex justify-end gap-2">
            ...
            <Button
              type="button"
              className="bg-vermelho text-white hover:cursor-pointer"
              onClick={() => confirmMove()}
            >
              Confirmar
            </Button>
