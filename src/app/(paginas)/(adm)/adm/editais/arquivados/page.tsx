...
export default function EditaisArquivados() {
  ...
  const [editalParaVisualizar, setEditalParaVisualizar] =
    useState<EditalArquivo | null>(null);
  const [editaisArquivadosFiltrados, setEditaisArquivadosFiltrados] = useState<
    Edital[]
  >([]);
  ...
  function filtrarEditais() {
    setCarregando(true);
    const editaisFiltrados = editaisArquivados.filter(
      (edital) =>
        edital.name &&
        edital.name.toLowerCase().includes(termoBusca.current.toLowerCase()),
    );
  ...
  function traduzirStatusEdital(status: string | undefined) {
    switch (status) {
      case "PENDING":
        return (
          <span className="flex items-center gap-2 bg-[#99A1AF] px-3 py-1 rounded-sm w-fit">
            <Copy size={14} /> Rascunho{" "}
          </span>
        );
      case "IN_REVIEW":
        return (
          <span className="flex items-center gap-2 bg-[#FF0000] px-3 py-1 rounded-sm w-fit">
            <Edit size={14} /> Em construção
          </span>
        );
      case "UNDER_CONSTRUCTION":
        return (
          <span className="flex items-center gap-2 bg-[#656149] px-3 py-1 rounded-sm w-fit">
            <Edit size={14} /> Em construção
          </span>
        );
      case "COMPLETED":
        return (
          <span className="flex items-center gap-2 bg-[#006400] px-3 py-1 rounded-sm w-fit">
            <Check size={14} />
            Concluído
          </span>
        );
    ...
    <div className="flex flex-col gap-5">
      ...
      {!carregando ? (
        editaisArquivadosFiltrados.length > 0 ? (
              ...
              <table className="w-full border-separate border-spacing-0">
                ...
                <tbody>
                  {editaisArquivadosFiltrados.map((edital) => (
                    ...
                    >
                      <td className="px-4 py-3">{edital.name}</td>

                      <td className="px-4 py-3">{edital.identifier}</td>
                          ...
                          <Dialog>
                            ...
                            <DialogContent className="rounded-2xl">
                              ...
                              <DialogDescription className="text-md">
                                Tem certeza que deseja desarquivar o edital{" "}
          ...
          <div className="text-gray-400 text-2xl text-center animate-pulse">
