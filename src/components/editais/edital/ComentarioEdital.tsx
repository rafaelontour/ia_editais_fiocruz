...
export default function ComentarioEdital({
  ...
  const [abrirDialogAdicionar, setAbrirDialogAdicionar] =
    useState<boolean>(false);
  const [dialogEditarComentario, setDialogEditarComentario] = useState<
    string | null
  >(null);
  ...
  async function enviarComentario() {
    ...
    const resposta = await fazerComentarioEditalService(edital?.id, {
          ...
          <div className="flex items-center justify-between rounded-md py-3">
            ...
            {temComentarios &&
              (((edital?.history?.[0]?.status === "PENDING" ||
                ...
                ((edital?.history?.[0]?.status === "WAITING_FOR_REVIEW" ||
              ...
              <div className="flex flex-col h-screen justify-between items-center gap-10">
                {mostrarFormulario ? (
                  <div className="flex flex-col gap-4 w-full px-4">
                    ...
                    {enviandoComentario ? (
                      <div
                        className={`flex w-full justify-center items-center rounded-md py-2 bg-vermelho ${enviandoComentario && "cursor-not-allowed bg-red-400"}`}
                      >
                      ...
                      <div key={index} className="flex flex-col gap-2">
                        ...
                        <div className="flex items-center gap-5">
                          <div className="flex rounded-full min-w-14 h-14 bg-gray-300 justify-center items-center">
                            {item.author?.icon ? (
                              <img
                                src={urlBase + item.author.icon.file_path}
                                alt="Foto de perfil"
                          ...
                          <div className="flex flex-col gap-2 relative w-full">
                            ...
                            {(usuario?.access_level === "ADMIN" ||
                              ...
                              <div className="flex items-center gap-2 absolute right-0 top-0">
                                <Dialog
                                  open={dialogEditarComentario === item.id}
                                  onOpenChange={(open) =>
                                    setDialogEditarComentario(
                                      open ? item.id! : null,
                                    )
                                  }
                                >
                                  ...
                                  <DialogContent
                                    className={`${enviandoComentario && "cursor-not-allowed"}`}
                                    onCloseAutoFocus={() => resetComentario()}
                                  >
                                    <DialogHeader>
                                      <DialogTitle>
                                        Editar comentário
                                    ...
                                    <DialogFooter
                                      className={`${enviandoComentario && "pointer-events-none"}`}
                                    >
                                      <DialogClose>
                                        <BotaoCancelar />
                                      </DialogClose>
                                ...
                                <Dialog
                                  ...
                                  onOpenChange={(open) =>
                                    setDialogComentario(open ? item.id! : null)
                                  }
                                >
                                  <DialogTrigger asChild>
                                    <Button
