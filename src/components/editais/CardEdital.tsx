  ...
  function verificarTextoAcao(acao: string, log: Log) {
    switch (acao) {
          ...
          <ul className="flex flex-col gap-1 list-none">
            {log.description.includes("Alterou Nome") && (
              <li>
                O usuário{" "}
                <i className="font-bold">{log.user.username?.split(" ")[0]}</i>{" "}
                alterou o nome do documento{" "}
                {log.description.split("Alterou Nome")[1].split(";")[0]}.
              </li>
            )}
            ...
            {log.description.includes("Identifier") && (
              <li>
                O usuário{" "}
                <i className="font-bold">{log.user.username?.split(" ")[0]}</i>{" "}
                alterou o identificador do edital.
              </li>
            )}
            ...
            {log.description.includes("Editors") && (
              <li>
                O usuário{" "}
                <i className="font-bold">{log.user.username?.split(" ")[0]}</i>{" "}
                alterou os responsáveis do edital.
              </li>
            )}
            ...
            {log.description.split("para")[0].includes("Moveu") && (
              ...
              >
                ...
                <p>
                  ...
                  moveu {log.description.split("Moveu")[1]}.
                </p>
              </li>
            )}
          </ul>
      ...
      case "DELETE":
        return (
          <p>
            O documento foi excluído pelo usuário{" "}
            <i className="font-bold">{log.user.username?.split(" ")[0]}</i>
          </p>
        );
      default:
  ...
  return (
    <div
      ...
      title={
        lista.includes(edital.id)
          ? "Aguarde o processamento do edital para ver o resultado"
          : edital.name
      }
      style={style}
      className={`
    ...
    >
      ...
      <div
        {...attributes}
        {...listeners}
        className={`${lista.includes(edital.id) && "hidden"} h-12 teste ${cor()} rounded-t-sm flex items-center justify-center`}
      >
        <span
          className={`
      ...
      <div className="grid grid-cols-1 relative p-3 overflow-x-hidden">
        <div className="flex flex-col-reverse items-start justify-between min-w-0 wrap-break-word relative">
          <h3
            ...
            className="font-semibold text-xl min-w-0 wrap-break-word"
          >
            {edital.name}
          </h3>
          ...
          <div className="relative w-full flex justify-between">
            {edital.history &&
              edital.history.filter((h) => h.status === "UNDER_CONSTRUCTION")
                .length >= 2 &&
              edital.history.filter((h) => h.status === "WAITING_FOR_REVIEW")
                .length >= 1 && (
                <div className="mt-1 mb-2">
                  <Tooltip>
                    <div
                      className="flex items-center bg-black pl-3 pr-2 py-1 rounded-md"
                    ...
                    >
                      <TooltipTrigger>
                        {edital.history[0].status !== "COMPLETED" ? (
                          <span className="flex items-center gap-1 text-sm font-bold text-white">
                            Retrabalhando{" "}
                            <IconProgressHelp className="mt-0.5" size={17} />
                          </span>
                        ) : (
                          <span className="flex items-center gap-1 text-sm font-bold text-white">
                            Retrabalhado{" "}
                      ...
                      <TooltipContent className="text-md">
                        {edital.history[0].status !== "COMPLETED" ? (
                          <span>
                            Este edital já esteve em fase de análise e voltou
                            para fase de construção em algum momento
                          </span>
                        ) : (
                          <span>
                            Este edital esteve em análise, mas voltou para
                            construção pelo menos 1 vez
                          </span>
                        )}
                      </TooltipContent>
                    </div>
              ...
              <div className="flex flex-col gap-2">
                <DialogHeader>
                  <DialogTitle className="text-3xl">
                    Logs do documento <strong>{edital.name}</strong>
                  </DialogTitle>
                </DialogHeader>
                ...
                {!carregandoLogs ? (
                  logsPorData && Object.keys(logsPorData).length > 0 ? (
                            ...
                            {logs.map((log: Log) => (
                              ...
                              >
                                ...
                                <div className="flex w-full h-full items-stretch justify-between">
                                  <div
                                    className={`p-5 ${log.action !== "CREATE" && "ml-7"} w-full`}
                    ...
                    <div className="h-12 w-full flex items-center bg-zinc-300 p-2 rounded-md justify-center">
                      <span className="text-md pointer-events-none italic">
                        Nenhum log encontrado para este edital
                      </span>
                    </div>
                  )
                ) : (
                          ...
                          <Dialog>
                            ...
                            <DialogContent className="rounded-2xl">
                              ...
                              <DialogDescription className="text-md">
                                Tem certeza que deseja arquivar o edital{" "}
                                <span className="font-bold">{edital.name}</span>
                                ?
                              </DialogDescription>
                            ...
                            <DialogContent className="rounded-2xl">
                              <DialogHeader>
                                ...
                                <DialogDescription className="text-[14px] text-vermelho">
                                  Após a exclusão, não será possível recuperar
                                  os dados desse edital e análise realizada
                                </DialogDescription>
                              </DialogHeader>

                              <DialogFooter>
                                <DialogClose className="border bg-slate-300 px-3 py-1 rounded-sm hover:cursor-pointer">
                                  Cancelar
                                </DialogClose>
