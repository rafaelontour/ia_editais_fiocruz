...
const schemaEdital = z.object({
  ...
  identificador: z
    .string()
    .min(1, "O número de identificação do edital é obrigatório"),
...
export default function AdicionarEdital({
  ...
  const [tipificacoesSelecionadas, setTipificacoesSelecionadas] = useState<
    Tipificacao[] | []
  >([]);
  const [responsaveisEdital, setResponsaveisEdital] = useState<
    UsuarioUnidade[]
  >([]);
  ...
  async function enviarEdital(data: formData) {
    ...
    if (resposta !== 201) {
      if (resposta === 409) {
        toast.error("Erro ao enviar edital!", {
          description: "Já existe um edital cadastrado com esse número!",
        });
        return;
      }
      toast.error("Erro ao enviar edital!");
    ...
    toast.info("Edital enviado!", {
      description:
        "O edital está sendo processado e em breve você poderá ver o resultado!",
    });
          ...
          <form onSubmit={handleSubmit(enviarEdital)}>
            <div className="space-y-6">
              <div className="flex flex-col gap-3">
                <div className="flex gap-3">
                  ...
                  <div className="flex w-1/2 flex-col gap-3">
                    <Label htmlFor="tipe" className="text-lg">
                      Tipificações
                    </Label>
                    <Controller
                      name="tipificacoes"
                                ...
                                .filter(
                                  (t) =>
                                    !tipificacoesSelecionadas.some(
                                      (tipificacao) => tipificacao.id === t.id,
                                    ),
                                )
                                .map((tipificacao) => (
                                  <SelectItem
                  ...
                  <div className="flex flex-col gap-3 w-full">
                    ...
                    <div
                      className={`
                                                ...
                                                `}
                    >
                      {tipificacoesSelecionadas
                        .filter((t) =>
                          tipificacoes.find((tp) => tp.id === t.id),
                        )
                        .map((t: Tipificacao) => (
                          <div
                            key={t.id}
                          ...
                          >
                            <div
                              ...
                              onClick={() => {
                                ...
                                setValue(
                                  "tipificacoes",
                                  novaLista.map((tp) => tp.id),
                                );
                              }}
                            >
                              <div
                                className="flex items-center h-full"
                                title="Remover tipificação"
                              >
                                <span
                                  className="
              ...
              <div className="flex flex-row gap-3 w-full">
                <div className="flex flex-col gap-3 w-1/2">
                  <Label htmlFor="responsavel" className="text-lg">
                    Responsável
                  </Label>
                  <Controller
                    name="responsavel"
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        onValueChange={(value) => {
                          ...
                          if (responsaveisEdital.find((u) => u.id === value))
                            return;
                          setResponsaveisEdital((anteriores) => [
                            ...anteriores,
                            usuarios?.find((u) => u.id === value)!,
                          ]);
                        }}
                      >
                        <SelectTrigger className="w-full">
                ...
                <div className="flex flex-col gap-3 w-1/2">
                  ...
                  <Input {...register("identificador")} id="date" />
                      ...
                      {responsaveisEdital.map((usuario: UsuarioUnidade) => (
                        ...
                        >
                          <div
                            ...
                            className="flex h-full"
                            onClick={() => {
                              const novaLista = responsaveisEdital.filter(
                                (u) => u.id !== usuario.id,
                              );
